...
def affine_backward(dout, cache):
  ...
  dx_inp = np.dot(dout,w.T)
  dx = dx_inp.reshape(x.shape)
  dw = np.dot(x_inp.T,dout)
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    #         the 'cache' variable.
    # ================================================================ #
    arbiBatch_mean = np.mean(x, axis = 0)
    arbiBatch_var = np.var(x, axis = 0)
       
    running_mean = momentum * running_mean + (1 - momentum) * arbiBatch_mean
    running_var = momentum * running_var + (1 - momentum) * arbiBatch_var
    bn_param['running_mean'] = running_mean
    bn_param['running_var'] = running_var
                    
    arbiBatch_norm = (x - arbiBatch_mean) / np.sqrt(arbiBatch_var + eps)
    out = gamma * arbiBatch_norm + beta
        
    cache = {
            'x_minus_mean': (x - arbiBatch_mean),
            'arbiBatch_norm': arbiBatch_norm,
            'gamma': gamma,
            'inv_var': 1.0 / np.sqrt(arbiBatch_var + eps),
            'sqrt_var': np.sqrt(arbiBatch_var + eps)}
...
def batchnorm_backward(dout, cache):
  ...
  arbiBatch_norm = cache.get('arbiBatch_norm')
  ...
  dgamma = np.sum(dout * arbiBatch_norm, axis = 0)
